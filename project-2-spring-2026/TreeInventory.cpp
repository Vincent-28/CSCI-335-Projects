...
{
std::unordered_set<Item> result; //create a set to store all the itesm that fall within the range   
    if (Comparator::lessThan(end, start)) { //if the end is less than the start then the range is invalid so return an empty set
        return result;
    }
    queryHelper(start, end, items_.root(), result); //call the helper to traverse the avl tree
    return result; //return all the items that were found in the range.
}
...
void Inventory<Comparator, Tree>::queryHelper(const Item& start, const Item& end, const Node* root, std::unordered_set<Item>& result) const
{
    ...
    if (Comparator::lessThan(root->value_, start)) { //if current node value is less than start, then everything in the left subtree is also less so skip and go right
        queryHelper(start, end, root->right_, result);
    }
    else if (Comparator::lessThan(end, root->value_)) { //if current node's value is greater than end, then skip everuthing in the right subtree and go left
        queryHelper(start, end, root->left_, result);
    }
    else {
        result.insert(root->value_);
        queryHelper(start, end, root->left_, result); //else, add current item to result
        queryHelper(start, end, root->right_, result); //explore both sides because valid items may exist on both sides still
    }
}

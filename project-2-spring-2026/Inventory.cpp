...
{
    /*
    here we are checking if an item with the same name 
    already exists and if it does, do not add the duplicate
    */
    if (contains(target.name_)) { 
        return false;
    }
    items_.insert(items_.end(), target); //insert the item at the end of the container
    weight_ += target.weight_; //add the item's weight to the total inventory weight
    return true;
}
...
bool Inventory<Comparator, Container>::contains(const std::string& name) const
{
    auto itr = std::find_if(items_.begin(), items_.end(), [&name](const Item& i) {
        return i.name_ == name; //for each line, check if its nme matches the target name
    });

    if (itr == items_.end()) { //if the iterator reached the end then the item was not found
        return false;
    }
    return true;
}
...
bool Inventory<Comparator, Container>::discard(const std::string& name)
{
    auto itr = std::find_if(items_.begin(), items_.end(), [&name](const Item& i) {
        return i.name_ == name;
    }); //search for the item witht he given name 

    if (itr == items_.end()) { //if we reached the end, the item was not found
        return false;
    }
    weight_ -= itr->weight_; //subtract the weight from the total inventory weight
    items_.erase(itr); //erase
    return true;
}

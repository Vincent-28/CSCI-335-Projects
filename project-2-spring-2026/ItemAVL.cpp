...
bool ItemAVL<Comparator>::contains(const std::string& target, const Node* subroot) const
{
    ...
    if(subroot == nullptr) { //base case
        return false;
    }
    if (subroot->value_.name_ == target) { //check if the current node's item name matches the target
        return true;
    }
    return contains (target, subroot->left_) || contains(target, subroot->right_); //recursively search in both left and right and if found then return true
}
...
{
    if (contains(target.name_)) { //check if an item with the same name already exists in the tree and if it does, do not insert any duplicates and return false
        return false;
    }
    insert(target, root_); //call the helper function to insert the item into the tree
    size_++; //increment since the insertion was successful 
    return true;
}

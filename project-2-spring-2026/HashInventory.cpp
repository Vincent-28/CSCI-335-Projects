...
{
    auto result = items_.insert(target); //try to insert the item into the unordered_set
    //you will get an itertor whcih is where the item is and a bool which is true if the insertion succeeded
    if (result.second) { //if the insertion was successful then the add the item's weight tot he total inventory weight
...
{
    Item temp(itemName, 0, NONE); //create temp item using just the name
    auto it = items_.find(temp); //try to find it in the unordered_set
    if (it == items_.end()) {
        return false; //return false if the item was not found
    }   
    weight_ -= it->weight_; //subtract it from the item's weight
    items_.erase(it); //erase it
    return true;
}
...
{
    std::unordered_set<Item> result; //creating a set to store the results
    if (Comparator::lessThan(end, start)) { //if end is less than start then the range is invalid
        return result;
    }
    for (const Item& item : items_) { //go through every item in the inventory
        if (Comparator::leq(start, item) && Comparator::leq(item, end)) {
            result.insert(item); //check if the item is in the range and if it is then add it to the result set
        }
    }
    return result;
}
